...
from custom_bandfeat import BandFeaturizer
...
def process_data(data_path, processed_path, processing_args):
    ...
    for index in range(0, len(target_data)):
        ...
        if processing_args["data_format"] != "db":
            ...
            data.pmg = pmg.io.ase.AseAtomsAdaptor.get_structure(ase_crystal)
        else:
            ase_crystal = ase_crystal_list[index]
            data.ase = ase_crystal
        if processing_args["read_bandstructure"] == True:
            bandstructure = pmg.electronic_structure.bandstructure.BandStructureSymmLine.from_dict(
                os.path.join(data_path, 'BS_'+structure_id + ".js")
            )
            data.pmg_bs = bandstructure
    ...
    if processing_args["dictionary_source"] != "generated":
        ...
        for index in range(0, len(data_list)):
            atom_fea = np.vstack(
                [
                    np.concatenate((one_hot_encode(sft.featurize(data_list[index].pmg, i)[3]), sft.featurize(data_list[index].pmg, i)))
    ...
    if processing_args["global_features"] == "True":
        for index in range(0, len(data_list)):
            # global structure features
            struct_feat = structureFeaturizer(data_list[index].pmg)
            # global bandstructure features
            band_feat = bandFeaturizer(data_list[index].pmg_bs)
            data_list[index].u = torch.Tensor(
                np.concatenate((struct_feat, band_feat))
            )
...
def structureFeaturizer(pmg):
    mft = MultipleFeaturizer([
        DensityFeatures(),
        StructureComposition(ElementProperty.from_preset("magpie"))
    ])
    struct_array = mft.featurize(pmg)
    return np.array(struct_array)

def bandFeaturizer(pmg):
    bf = BandFeaturizer()
    featuredict = bf.featurize(pmg)
    my_list = []
    # convert dict to list
    for value in featuredict.values():
        if isinstance(value, bool):
            my_list.append(1 if value else 0)
...
# define a function to get the one-hot encoding of 18 elements
